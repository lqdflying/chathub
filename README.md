...
Upgrade from any 1.x or 2.0.x image the same way as a patch: pull `:latest` or `2.0.4`, restart, let migrations run. [Release notes](https://github.com/lqdflying/chathub/releases/tag/v2.0.4) · [Wiki](https://github.com/lqdflying/chathub/wiki)
...
GitHub Actions builds `docker.io/lqdflying/chathub` on version tags. GA `v*.*.*` updates `:latest`. Canaries (`v*.*.*-canary.*`) do not. Current GA is **v2.0.4**; the next canary is `v2.0.5-canary.N`. [Release Workflow](https://github.com/lqdflying/chathub/wiki/Release-Workflow)
