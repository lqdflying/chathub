...
describe('transformToChatModelCards', () => {
  ...
  it('should handle azure real case', async () => {
    const sol = LOBE_DEFAULT_MODEL_LIST.find(
      (m) => m.id === 'gpt-5.6-sol' && m.providerId === 'azure',
    )!;
    const luna = LOBE_DEFAULT_MODEL_LIST.find(
      (m) => m.id === 'gpt-5.6-luna' && m.providerId === 'azure',
    )!;
    const gpt55 = LOBE_DEFAULT_MODEL_LIST.find(
      (m) => m.id === 'gpt-5.5' && m.providerId === 'azure',
    )!;

    const data = await transformToAiModelList({
      defaultModels: [sol, luna, gpt55],
      modelString:
        '-all,gpt-5.6-sol->id1=GPT-5.6 Sol,gpt-5.6-luna->id2=GPT-5.6 Luna,gpt-5.5->id3=GPT-5.5',
      providerId: 'azure',
      withDeploymentName: true,
    });

    expect(data).toEqual([
      { ...sol, config: { deploymentName: 'id1' }, displayName: 'GPT-5.6 Sol', enabled: true },
      { ...luna, config: { deploymentName: 'id2' }, displayName: 'GPT-5.6 Luna', enabled: true },
      { ...gpt55, config: { deploymentName: 'id3' }, displayName: 'GPT-5.5', enabled: true },
    ]);
  });
});
