export default {
  ...
  extendParams: {
    ...
    zhipuClearThinking: {
      desc: '关闭后启用 Preserved Thinking（thinking.clear_thinking=false），历史 reasoning_content 将原样回传以提升多轮推理质量。默认开启（按 Zhipu 服务端默认，从上下文中剥离历史思考以降低成本）。需与深度思考同时开启。',
    ...
    zhipuReasoningEffort: {
      desc: '仅 GLM-5.2+ 支持。max=深度思考（默认），high=增强思考，skip=跳过思考（映射到 API 的 minimal）。仅在思考开启时生效。',
      high: 'High',
      max: 'Max',
      skip: 'Skip',
