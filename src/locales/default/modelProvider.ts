export default {
    ...
    config: {
      ...
      openAICompatCache: {
        chatPromptCacheKey: {
          ...
          title: 'Chat 发送 prompt_cache_key',
        },
        chatSessionHeader: {
          desc: '为 /v1/chat/completions 请求发送 Session_id 请求头',
          title: 'Chat 发送 Session_id',
        },
        preset: {
          desc: '按已验证的内置模式写入整套缓存参数；手动修改后会切换为自定义',
          options: {
            custom: '自定义',
            promptKeyStore: 'Prompt key + store',
          },
          title: 'OpenAI 兼容缓存预设',
          tooltip:
            '已在 pptoken.org 与 apikl.ai 验证的内置模式。Chat Completions 发送 prompt_cache_key；Responses 发送自动生成的 prompt_cache_key 与 store:true；不发送 Session_id。其他网关组合请使用自定义。',
        },
        promptCacheKey: {
          options: {
            derived: '自动生成',
        ...
        responsesPromptCacheKey: {
          desc: '为 /v1/responses 请求生成并发送 prompt_cache_key',
          title: 'Responses prompt_cache_key',
        },
        responsesSessionHeader: {
          desc: '为 /v1/responses 请求发送 Session_id 请求头',
          title: 'Responses Session_id',
        },
        responsesStore: {
          desc: '控制 /v1/responses 的 store 字段；默认表示由运行时或服务商决定',
          title: 'Responses store',
        },
        store: {
          options: {
            default: '默认',
      ...
      openAICompatResponsesParams: {
        maxOutputTokens: {
          desc: '将模型最大输出设置作为 max_output_tokens 发送到 /v1/responses',
          title: 'Responses 发送 max_output_tokens',
        },
        maxTokens: {
        ...
        truncation: {
          ...
          title: 'Responses truncation',
        },
        verbosity: {
          desc: '控制文本详细程度参数在 /v1/responses 中的发送方式',
          options: {
            both: 'text.verbosity + 顶层 verbosity',
            off: '不发送',
            text: 'text.verbosity',
            topLevel: '顶层 verbosity',
          },
          title: 'Responses verbosity',
        },
      },
      responsesApi: {
        desc: '选择 OpenAI 兼容请求路径；部分代理只支持其中一种路径或参数组合',
        options: {
          chatCompletions: 'Chat Completions',
          responses: 'Responses API',
        },
        title: 'OpenAI 兼容 API 路径',
      },
      responsesState: {
  ...
  xaiOAuth: {
    connected: '已连接：{{email}}',
    connectedAnonymous: '已连接 SuperGrok',
    connectedPlan: '{{plan}} 套餐',
    denied: '授权被拒绝或失败，请重新登录。',
    deviceLoginPrerequisite:
      'xAI 设备码登录可能显示为 Grok Build。这是共享的非官方 Grok CLI 客户端，不是 ChatHub 官方应用。',
    expired: '设备码已过期，请重新登录。',
    expires: '会话将于 {{time}} 过期',
    fiveHourLeft: '5 小时额度剩余 {{percent}}%',
    fiveHourTitle: '5 小时额度',
    helpAria: '关于 SuperGrok 登录',
    hint: '登录后，xAI 聊天将使用你的 SuperGrok 额度，而不是 Console API Key。图像与语音 API 不在此版本。',
    monthlyTitle: '每月额度',
    openGrok: '打开 Grok 授权页',
    remainingPercent: '剩余 {{percent}}%',
    signIn: '使用 SuperGrok 登录',
    signOut: '退出登录',
    title: 'SuperGrok',
