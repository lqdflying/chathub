...
- **thinking** — MiniMax-M3 Chat Completions field: omit/`adaptive` keeps thinking on; `{ type: "disabled" }` skips thinking. Connectivity Check sends `thinking: { type: "disabled" }`, `stream: false`, and leaves `reasoning_split` at the chat default. Safari iOS previously cleared the Check spinner with no pass/fail even when Axiom showed `finishReason: stop` and hello text — Checker settles WebKit `Load failed` with text or reasoning-only output as pass (via `onAbort`, not shared `onErrorHandle`), keeps the component mounted during config refresh, and always shows pass or fail. Shared `fetchSSE` delivers `text: 'none'` reasoning immediately, flushes any 300ms reasoning buffer before `onAbort`, and puts the accumulated thinking string on `interrupt.reasoning` so a fast reasoning event plus WebKit abort is not classified as empty. When abort arrives with empty output (WebKit often throws `Load failed` after a complete short non-stream→SSE body), Checker does **not** settle fail yet. Preset tasks set `rawByteCaptureMax` so `fetchEventSource` keeps bounded raw chunks while reading — `response.clone()` shares the errored tee and cannot re-read. Recovery extracts `event: text` only (never dumps tool-only/raw SSE as assistant text, never races smooth queues), then `onFinish` runs; Checker’s `onError` must not overwrite a prior settle. Empty intentional aborts in shared `fetchSSE` remain abort-only so chat Stop-before-first-token is not turned into a provider error. M2.x cannot disable thinking. Official: [Chat Completions](https://platform.minimax.io/docs/api-reference/text-chat-openai).
