...
## 2026 built-in catalogue

Built-in model-bank cards with `releasedAt` before 2026-01-01 were removed from the picker (saved agent ids still go to the vendor). Cards whose official shutdown is already past were also dropped (for example OpenAI `gpt-5.2-chat-latest`). Missing `releasedAt` is filled from the vendor's current list rather than dropped.
...
Native OpenAI `gpt-6-astra` / `gpt-6-astra-*` tool turns (non-empty `tools`, or messages with `tool_calls` / `role: tool` / `function_call`) are routed to `/v1/responses` inside `LobeOpenAI` `handlePayload`. Structured tool selection through `generateObject()` (group supervisors and tRPC structured output with tools) uses the same Astra matcher via `generateObject.useResponseModels` because that path never runs chat `handlePayload`. This is native-adapter routing, not membership in `responsesAPIModels`. Tool-free native Astra `chat()` stays on Chat Completions. The `openaicompatible` provider still defaults to Chat Completions and uses the user-facing API-mode radio. Browser `getChatCompletion` and Graphile `buildConversationChatPayload` share `resolveOpenAICompatibleChatRoute`; group-supervisor `generateObject` sets `responseApi: true` only when that radio is Responses. Official: [Using GPT-6 Astra](https://developers.openai.com/api/docs/guides/latest-model) (Chat Completions is supported; tool calling requires Responses).

Azure OpenAI / Azure AI stay on Chat Completions. Microsoft requires `reasoning_effort: none` when GPT-5.6 Chat Completions includes function tools; otherwise the request is rejected. ChatHub forces `none` on those tool chats using the catalog id, not the outgoing deployment alias: the web API validates `trustedCatalogModel` server-side, and browser-direct (Browser Request) mode reuses the locally resolved catalog/deployment mapping. Tool-free effort is left unchanged. This is a documented quality tradeoff until Azure Responses is wired.
...
Fetched `glm-4.7*` infers Preserved Thinking only (`enableReasoning` is not injected) because the Zhipu adapter forces thinking for that family.
...
Keepalives are emitted only between complete SSE frames, never between an `id`, `event`, and `data` sequence. The response uses `Cache-Control: no-cache, no-transform` and `X-Accel-Buffering: no`; ChatHub's EventSource parser and `fetchSSE` leftover recovery both ignore comment-only frames, so a heartbeat-only body is not assistant text. Canceling the downstream body or the original request aborts the linked upstream request. If opening the deferred provider stream fails, its normalized provider metadata is converted into the normal terminal SSE error event instead of surfacing as an untyped browser `TypeError: Load failed`.
...
`zhipu` is a first-class OpenAI-compatible provider built with the shared factory. Its default ChatHub route is the China endpoint `https://open.bigmodel.cn/api/paas/v4`; deployments targeting the international `https://api.z.ai/api/paas/v4/` endpoint must configure the base URL explicitly. `glm-5.3` and `glm-5.2` are enabled flagship text models with a `1_048_576`-token context window; `glm-5.3-flash` is the enabled native-multimodal sibling. Per the Zhipu API ref, GLM-5.3/5.3-Flash/5.2/5.1/5/5-turbo text models support 128K (131_072) max output, and GLM-5V-Turbo supports 128K (131_072); the shipped `maxOutput` values match. Official: [GLM-5.3](https://docs.z.ai/guides/llm/glm-5.3), [GLM-5.3-Flash](https://docs.z.ai/guides/vlm/glm-5.3-flash), [Deep Thinking](https://docs.z.ai/guides/capabilities/thinking).
...
- **Gateway-safe thinking object** — the adapter never sends the literal `thinking.type: "enabled"` because some OpenAI-compatible GLM gateways (LiteLLM → vLLM) hard-reject that string with HTTP 400 (probe-verified 2026-08-06), while the official API defaults `thinking.type` to `enabled` anyway (https://docs.z.ai/guides/capabilities/thinking). The rejection is **path- and backend-dependent**: the same gateway returned 400 for `{type:"enabled"}` on plain (no-tools) requests but 200 on tool requests in one probe round, and 400 for both in an earlier round (its backend pool changed between rounds) — so the contract avoids the literal entirely rather than relying on observed leniency. The wire translation is: thinking **ON** → omit the `thinking` field entirely (behavior-identical upstream); thinking **OFF** → `{ type: 'disabled' }` (accepted on GLM-5.2 and older); **Preserved Thinking** → `{ clear_thinking: false }` with **no** `type` key (type defaults to enabled upstream; gateways accept the type-less object, verified plain + tools + a 2-turn replayed-`reasoning_content` conversation). Anthropic-style `budget_tokens` and Moonshot-style `keep` from the service layer are always stripped. GLM-4.7 forces thinking per Zhipu docs ("GLM-4.7 will think compulsorily"), so it ships **no** `enableReasoning` toggle — but `clear_thinking` is a documented GLM-4.5+ capability orthogonal to forced thinking, so `glm-4.7` ships `zhipuPreservedThinking` only; the service layer has a dedicated zhipu-only branch that attaches `thinking: { type: 'enabled', clear_thinking: false }` when Preserved Thinking is on, which the runtime then translates to the type-less form. **GLM-5.3 and GLM-5.3-Flash also force thinking**: `thinking.type: "disabled"` is an API error. Those cards ship `zhipuReasoningEffort` + `zhipuPreservedThinking` without `enableReasoning`. If a payload still asks for disabled (for example a leftover 5.2 chatConfig or a connectivity probe), `buildZhipuPayload` treats thinking as on and omits the field. Connectivity Check keeps `thinking.disabled` for glm-5.2 and older; for `glm-5.3*` it omits `thinking` and sends `reasoning_effort: "low"` (Checker already accepts reasoning-only output). Title, translation, History Compress, and assistant-memory simple completions (`buildSimpleCompletionSampling`) use the same `glm-5.3*` contract: they do not emit `thinking.disabled`; they send `reasoning_effort: "low"` so the vendor default `max` cannot consume the bounded summary budget.
- **reasoning_effort** — forwarded only on `glm-5.2` and above, and only when thinking is enabled. GLM-5.2: the chat service maps the UI `skip` value to the API **`none`** value (NOT `minimal` — the same gateways reject `minimal` with HTTP 400; `none` is probe-verified to skip thinking); the runtime forwards that value verbatim. GLM-5.3 / GLM-5.3-Flash: official values are only `low` | `high` | `max` (default `max`); any other value 400s. The gear shows Low/High/Max; leftover `skip`/`none`/`minimal` map to `low` (vendor migration: disabled → enabled + `low`). The runtime clamps 5.3 effort to that set before the wire.
...
- **web_search tool** — injected into the `tools` array when `enabledSearch` is set, using `{ type: 'web_search', web_search: { search_engine: 'search_pro_jina', enable: true } }`. Reachability requires the model card to declare `settings.searchImpl: 'params'`; the shipped text cards (`glm-5.3`, `glm-5.2`, `glm-5.1`, `glm-5`, `glm-5-turbo`) set it; `glm-5.3-flash` and `glm-5v-turbo` do not (vision request schema has no `web_search` tool).
...
The default model list ships 7 GLM-5.x cards (`glm-5.3`, `glm-5.3-flash`, `glm-5.2`, `glm-5.1`, `glm-5`, `glm-5-turbo`, `glm-5v-turbo`). `glm-5.3` / `glm-5.3-flash` carry `zhipuReasoningEffort` + `zhipuPreservedThinking` (thinking forced, no `enableReasoning`); `glm-5.2` carries `enableReasoning` + `zhipuReasoningEffort` + `zhipuPreservedThinking`; the rest carry `enableReasoning` and `zhipuPreservedThinking`. Fetched leftover `glm-4.7*` still force thinking at runtime (`packages/model-runtime/src/providers/zhipu/index.ts`) and infer **Preserved Thinking only** (no `enableReasoning`). Fetched Zhipu models receive inferred `extendParams` at read time in `packages/database/src/repositories/aiInfra/index.ts` because the remote model table cannot persist `settings.extendParams`; fetched `glm-5.3*` ids get `['zhipuReasoningEffort', 'zhipuPreservedThinking']` only. Fetched `glm-5.3-flash` also gets vision/video from `MODEL_LIST_CONFIGS.zhipu` keywords.
...
- **Fetch** — `client.models.list()` plus `MODEL_LIST_CONFIGS.mimo`; drop tts/asr/voiceclone/voicedesign ids. `mimo-v2.6-pro`, `mimo-v2.6-flash`, `mimo-v2.6-pro-ultraspeed`, and exact `mimo-v2.5` get vision/video. `mimo-v2.5-pro` stays text-only. Xiaomi removes `mimo-v2.5-pro` and `mimo-v2.5` at 2026-10-21 10:00 UTC+8. Fetched V2.5 and V2.6 ids infer `enableReasoning` at read time.
...
- **thinking** — MiniMax-M3 Chat Completions field: omit/`adaptive` keeps thinking on; `{ type: "disabled" }` skips thinking. Connectivity Check sends `thinking: { type: "disabled" }`, `stream: false`, `responseMode: "json"`, and leaves `reasoning_split` at the chat default. The browser reads `application/json` with `Response.json()` ([MDN](https://developer.mozilla.org/en-US/docs/Web/API/Response/json)); it must not wrap that completed MiniMax body as a few SSE frames. Safari iOS often throws `TypeError: Load failed` on that short `text/event-stream` without delivering bytes even when Axiom shows `finish_reason: stop` and hello text. Chat stays on SSE. Before transport, MiniMax's context guard uses the serialized request's UTF-8 byte count as a conservative proof that a short probe fits; this avoids starting the tokenizer worker for the one-message check because that worker may be unavailable in mobile Safari/PWA contexts. Larger requests retain exact/estimated token trimming and fall back conservatively to byte count if tokenization fails. A MiniMax check passes on assistant text, reasoning, or a successful terminal JSON envelope (`base_resp.status_code` zero plus a choice `finish_reason`); the last signal keeps connectivity semantics correct when a mobile client exposes no answer text. Failed empty JSON checks report only content-free shape metadata: baked client version, browser/server transport, media/value types, lengths, keys, finish reason, and provider status. Pre-transport failures include a content-free failure reason and error class. This also makes a stale mobile canary immediately visible without logging provider output. Runtimes that ignore `responseMode: json` (Anthropic Messages) still return SSE; the client falls back to `fetchSSE`. Checker still settles WebKit abort with text or reasoning-only output as pass when that fallback runs. M2.x cannot disable thinking. Official: [Chat Completions](https://platform.minimax.io/docs/api-reference/text-chat-openai).
...
DeepSeek, MiniMax, Moonshot, Xiaomi MiMo, and Zhipu use provider-specific model fetchers rather than a raw generic list. The fetchers call the shared model parser with provider configs, and the database repository adds read-time-only `settings.extendParams` for fetched models where the remote model table cannot store option-panel settings. This keeps fetched DeepSeek V4, MiniMax M-series, Xiaomi MiMo V2.5 / V2.6, and OpenAI/Azure GPT-5.x models usable in the model option panel, while Moonshot Kimi K2.7 Code and Kimi K3 remain reasoning-capable without a toggle because the provider forces thinking/preserved reasoning. Fetched Zhipu GLM reasoning models infer `enableReasoning` + `zhipuPreservedThinking` (plus `zhipuReasoningEffort` for `glm-5.2` and above); fetched `glm-5.3*` ids get `zhipuReasoningEffort` + `zhipuPreservedThinking` only (thinking forced). Fetched leftover `glm-4.7*` infer Preserved Thinking only and still force thinking in the Zhipu adapter. Fetched `deepseek-flash` (and leftover `deepseek-v4-flash` / `deepseek-v4-flash-vision-exp`) gets vision from the DeepSeek `flash` / `vision` keywords. Fetched K3 / K2.7 variants receive reasoning, vision, and video capabilities from the Moonshot keyword normalizer when an exact built-in card is unavailable. Fetched leftover `kimi-k2.5` still infers `enableReasoning`.

Simple-completion sampling (`src/helpers/contextCompaction.ts`) reads `LOBE_DEFAULT_MODEL_LIST` independently of fetched DB cards. Native DeepSeek saved ids `deepseek-v4-flash` and `deepseek-v4-flash-vision-exp` reuse the `deepseek-flash` card for thinking-off and the 1M summarizer window; the request still sends the saved id. Foreign providers do not inherit that alias.
