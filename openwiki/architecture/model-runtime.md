...
The `openaicompatible` provider intentionally uses a fixed, non-editable model list instead of exposing arbitrary model fetching. Its chat catalog clones `gpt-5.6-sol` and `gpt-5.5` from the native OpenAI model bank, preserves their option settings, and disables the native-search ability so search remains an explicit compatible-provider option. Both compatible chat cards override the native `1_050_000`-token context window with the shared `258_000`-token compatibility limit while retaining the `128_000` maximum output. The override is provider-scoped and therefore controls token estimates and automatic context-compaction watermarks only when the active provider is `openaicompatible`. `gpt-image-2` remains the fixed image model.
