...
- **No thinking object** — the adapter never sends `thinking`, `reasoning_effort`, or `clear_thinking`. Per the Zhipu thinking guide, `thinking.type: 'enabled'` is the **default** on thinking-capable models (GLM-5.x, GLM-4.6/4.7; GLM-4.7 forces thinking), so omitting the field is behavior-identical to enabling it on the official API. Omitting also avoids HTTP 400s from some OpenAI-compatible GLM gateways (LiteLLM → vLLM) that hard-reject the `thinking` object. Consequently ChatHub ships **no** thinking / reasoning-effort / Preserved-Thinking toggles for any GLM model, and the service layer skips the entire `enableReasoning` block for `zhipu`. Because Preserved Thinking is never in play, the adapter always strips the internal `reasoning` field and any bare `reasoning_content` from assistant history so the shared `convertOpenAIMessages` builder does not re-inject it and waste input tokens.
...
The default model list ships 8 GLM cards (`glm-5.2`, `glm-5.1`, `glm-5`, `glm-5-turbo`, `glm-4.7`, `glm-4.6`, `glm-4.5`, `glm-5v-turbo`). None carries a reasoning `extendParams` entry (thinking is always on by default and no thinking params are sent), so GLM models show no model-gear reasoning toggles; the text cards keep only `settings.searchImpl: 'params'` for web search. Fetched Zhipu models likewise infer no reasoning `extendParams` at read time in `packages/database/src/repositories/aiInfra/index.ts`.
