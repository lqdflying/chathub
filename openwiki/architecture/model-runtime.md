...
Native OpenAI `gpt-6-astra` / `gpt-6-astra-*` tool turns (non-empty `tools`, or messages with `tool_calls` / `role: tool` / `function_call`) are routed to `/v1/responses` inside `LobeOpenAI` `handlePayload`. This is native-adapter routing, not membership in `responsesAPIModels`. Tool-free native Astra stays on Chat Completions. The `openaicompatible` provider still defaults to Chat Completions and uses the user-facing API-mode radio. Official: [Using GPT-6 Astra](https://developers.openai.com/api/docs/guides/latest-model) (Chat Completions is supported; tool calling requires Responses).
