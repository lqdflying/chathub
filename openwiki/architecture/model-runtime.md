...
- **thinking** — MiniMax-M3 Chat Completions field: omit/`adaptive` keeps thinking on; `{ type: "disabled" }` skips thinking. Connectivity Check sends `thinking: { type: "disabled" }` and leaves `reasoning_split` at the chat default so the probe matches normal mobile chat streaming (Safari iOS previously hung when the probe forced `reasoning_split: false`). M2.x cannot disable thinking. Official: [Chat Completions](https://platform.minimax.io/docs/api-reference/text-chat-openai).
