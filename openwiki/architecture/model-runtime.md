...
- **structured output / generateObject** — Xiaomi documents `response_format.type: json_object` plus an explicit JSON instruction in messages ([structured output](https://mimo.mi.com/docs/en-US/quick-start/usage-guide/text-generation/structured-output)). ChatHub still advertises `structuredOutput` on both cards. `generateObject` (group supervisor and other structured jobs) is shaped by `shapeMimoGenerateObjectRequest`: omit `user`, coerce `tool_choice` to `auto`, and rewrite `json_schema` to `json_object` with the schema injected into messages. The inherited factory path would otherwise send undocumented `json_schema` / `user` / `tool_choice: required`.
- **web_search** — on pay-as-you-go (`api.xiaomimimo.com`), when `enabledSearch` is set, append `{ type: 'web_search' }` to `tools`. Do not send `force_search` by default. Do not auto-disable thinking for search/tools. On Token Plan hosts (`token-plan*.xiaomimimo.com`) omit native `{ type: web_search }` (and strip it if already present) because Token Plan returns `400` `webSearchEnabled is false` until Xiaomi Console → Plugin Management enables Web Search; function/MCP tools still go through. Search routing also forces `useModelSearch=false` for Token Plan (Settings endpoint or `MIMO_PROXY_URL`) so ChatHub browsing/MCP stays on even if the native-search toggle was saved on. The search panel hides the native toggle for Token Plan endpoints.
