...
- Overflow section headers are a full line `[date:YYYY-MM-DD]`. Ordinary body lines that are exactly `[YYYY-MM-DD]` or `[date:YYYY-MM-DD]` are backslash-stuffed (`\[…]`) so they are not reparsed as days. A later cap rewrites legacy overflow that used bare `[YYYY-MM-DD]` markers; only dates inside that card's range tag are accepted as parts.
