...
Live token-popover work must not copy the full topic on every keystroke. `useEstimatedContextUsage` fingerprints the map with `createConversationMessageRevision` (ids / `updatedAt` / content lengths), debounces draft-driven window serialize by `CONTEXT_ESTIMATE_INPUT_DEBOUNCE_MS` (300ms), and tokenizes the uncapped topic string only when that revision or the input template changes. `EstimatedContextUsageProvider` runs that hook **once** for the token badge and `ContextAutoCompactWatcher`. AutoScroll / group-token follow-output use `mainAIFollowOutputRevision` (same fingerprint plus last reasoning length) instead of joining bodies. Chat rows and the desktop minimap subscribe to the raw `messagesMap` object / display IDs so typing does not clone every message. Virtuoso `increaseViewportBy` is one desktop viewport, or half of `visualViewport.height` (capped) on mobile ([MDN Visual Viewport](https://developer.mozilla.org/en-US/docs/Web/API/Visual_Viewport_API)). While the list is scrolling, bubbles keep chrome and text but skip Shiki / mermaid ([Virtuoso scroll handling](https://virtuoso.dev/react-virtuoso/virtuoso/scroll-handling/)). Light markdown waits `LIGHT_SCROLL_ACTIVATE_MS` (150ms) of sustained `isScrolling` and freezes the last idle row height, because Virtuoso also reports scrolling on layout updates ([issue #114](https://github.com/petyosi/react-virtuoso/issues/114)) and a Shiki↔`<pre>` swap at a different height loops the list (PC hover shiver). A [ResizeObserver](https://developer.mozilla.org/en-US/docs/Web/API/ResizeObserver) refreshes that idle height when width or a late child changes the natural box; it never samples while light markdown is on. Light markdown runs only when that height is known, and `minHeight` is held for `LIGHT_SCROLL_RELEASE_MS` after the wheel stops so restoring Shiki cannot collapse the row. LobeChat already moved `gpt-tokenizer` off the main thread after 10k+ token hitch ([discussion #1029](https://github.com/lobehub/lobe-chat/discussions/1029), [PR #3754](https://github.com/lobehub/lobe-chat/pull/3754)); this fork still cannot post the whole 400k-token transcript into that worker on each character.
