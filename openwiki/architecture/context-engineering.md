...
- Overflow section headers are a full line `[date:YYYY-MM-DD]`. Newly written overflow cards begin with a magic line `[overflow:v1]` (stripped in the Memory editor) so grammar is not inferred from editable body text. Ordinary body lines that are exactly `[YYYY-MM-DD]` or `[date:YYYY-MM-DD]` are backslash-stuffed (`\[…]`) so they are not reparsed as days. Lines before the first accepted heading stay with the range-start part. Pre-sentinel documents: if any in-range `[date:…]` heading exists, parse as canonical (bare date lines stay body text); otherwise parse bare `[YYYY-MM-DD]` headings as legacy. Do not vote with body-line counts. Overflow truncation measures the stuffed serialized body, including the sentinel, so each card stays `<= ASSISTANT_MEMORY_MAX_CHARS`.
