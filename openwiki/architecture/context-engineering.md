...
- Overflow section headers are a full line `[date:YYYY-MM-DD]`. Newly written overflow cards begin with a magic line `[overflow:v1]` (stripped in the Memory editor and omitted from Copy). Ordinary body lines that are exactly `[YYYY-MM-DD]`, `[date:YYYY-MM-DD]`, or `[overflow:v1]` are backslash-stuffed (`\[…]`) so they are not reparsed as framing. The editor unescapes those lines, so a user-typed leading `[overflow:v1]` stays visible content; card save re-attaches the stored control marker from the previous body rather than consuming the typed line. Lines before the first accepted heading stay with the range-start part. Pre-sentinel documents are dual-parsed (canonical `[date:]` vs legacy bare `[YYYY-MM-DD]`); a parse is used only when its reconstructed range tag equals the outer card tag. If both match, prefer fewer parts, then the parse whose newest part contains the last non-empty source line. Do not classify unversioned cards by “any `[date:]` line.” Overflow truncation measures the stuffed serialized body, including the sentinel, so each card stays `<= ASSISTANT_MEMORY_MAX_CHARS`.
