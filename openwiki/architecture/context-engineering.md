...
newly folded days, stamp `[overflow:opaque-v2]` so a later cap cannot reparse the payload
as dated sections (even if a folded day contains `[date:day]` or `[day]`), and retain the
newest tail instead of rewriting it as a fake start-dated part that would drop the whole
range.
