...
- Overflow section headers are a full line `[date:YYYY-MM-DD]`. Ordinary body lines that are exactly `[YYYY-MM-DD]` or `[date:YYYY-MM-DD]` are backslash-stuffed (`\[…]`) so they are not reparsed as days. Lines before the first accepted heading stay with the range-start part (range cards are user-editable). Grammar is chosen by **which native heading is more common** among in-range lines (`[date:…]` vs bare `[YYYY-MM-DD]`), not by whichever marker appears first, so a canonical card stays canonical when the user prepends an ordinary `[YYYY-MM-DD]` line, and a legacy two-day card is not rewritten as a one-day card merely because day one contains `[date:YYYY-MM-DD]`. A later cap rewrites legacy overflow that used bare `[YYYY-MM-DD]` markers; only dates inside that card's range tag are accepted as parts. Overflow truncation measures the stuffed serialized body so each card, including escape characters, stays `<= ASSISTANT_MEMORY_MAX_CHARS`.
