...
- Connectivity Check for `openaicompatible` streams like chat (`stream: true`) so gateways can record First/TTFT. It still omits token-limit fields. Check passes on streamed text, reasoning, a completed Responses JSON envelope (`status: completed`), or an explicit SSE `stop` terminal (`stop` / `completed`). Clean EOF without that terminal is not a pass. SSE comment lines, including the server `: chathub-ping` heartbeat, are keep-alives and do not count as output. Incomplete or error terminals still fail. Native OpenAI Check remains JSON non-stream with the completed-envelope rule. MiniMax still uses its Chat Completions completed-envelope JSON rule.
