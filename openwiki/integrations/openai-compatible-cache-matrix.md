...
- A cache request record and its usage records share a 16-hex `requestHash`; usage records also include a stable bounded `responseHash` for repeated observations of the same provider response. Tool completion records use the same tool-call set hash and retain each tool's own `diagnosticId`.
