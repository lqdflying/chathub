...
- Connectivity Check for `openaicompatible` streams like chat (`stream: true`) so gateways can record First/TTFT. It still omits token-limit fields. Check passes on streamed text, reasoning, a completed Responses JSON envelope (`status: completed`), or a successful SSE finish (`type: done`) even when exposed text is empty. Incomplete or error terminals still fail. Native OpenAI Check remains JSON non-stream with the completed-envelope rule. MiniMax still uses its Chat Completions completed-envelope JSON rule.
