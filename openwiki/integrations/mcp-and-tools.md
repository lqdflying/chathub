...
MCP tool results cross a second browser-facing boundary when the chat saves them through `message.update`. These updates are isolated from the normal Lambda batch and reuse the MCP `diagnosticId`. `tool_persistence_rpc_started|complete|failed` records describe that Lambda request and its outgoing response; a browser-side parse/read/network failure is reported as `client_rpc_response_failed` with `rpcEndpoint: "lambda"`, `procedure: "message.update"`, and the attempt number. ChatHub retries this idempotent update once. If both confirmations fail, the valid result remains in the optimistic chat state and the model continuation proceeds, while the UI warns that a reload may lose the unsaved result.

Useful phase events include `tools_rpc_started|complete|failed`, `tool_persistence_rpc_started|complete|failed`, `client_cache_lookup`, `client_initialization_*`, `oauth_operation_*`, `transport_request_*`, `mcp_operation_*`, `call_tool_upstream_complete`, `call_tool_normalized`, `call_tool_complete`, and `client_rpc_response_failed`. `call_tool_upstream_complete` only means the MCP SDK returned; `call_tool_complete` is emitted after successful normalization and serialization.
