...
- Stale streamable-HTTP sessions are self-healing on both discovery and tool calls: `No valid session ID provided` maps to `NoValidSessionId`. `listTools` retries with a fresh client (up to 3 attempts); `callTool` evicts the poisoned cached client and retries exactly once. Tool-level failures (`isError` / `McpError`) are never replayed — the request may have reached the server and could be mutating. The replay condition is **transport-only** (`isTransportSessionRejection`): only a plain transport `Error` whose message carries the session phrase triggers eviction and retry. An SDK `McpError` — an application-level response the server already processed — is re-thrown as-is even when its message contains the session phrase, so a server-side rejection can never be silently replayed.
- Client initialization is bounded by `MCP_INITIALIZATION_TIMEOUT` (default 30s, read at call time); on deadline the half-open transport is closed best-effort and a structured `INITIALIZATION_TIMEOUT` error surfaces instead of waiting on undici/SDK defaults. `MCPClient.disconnect()` calls the SDK `Client.close()` (which also closes the transport) and falls back to `transport.close()` if the SDK close fails.
- The client cache is bounded: a 60s unref'd sweep evicts clients idle longer than `MCP_CLIENT_IDLE_TTL_MS` (default 30 min) and enforces `MCP_CLIENT_CACHE_MAX_SIZE` (default 100) in least-recently-used order; undici/network connection failures (`fetch failed`, `ECONNREFUSED`, `socket hang up`, …) also evict the cached client so the next call reconnects instead of reusing a zombie.
- A per-connection circuit breaker quarantines repeatedly failing servers: 3 consecutive transport-level failures (including SDK request timeouts) open the circuit for 60s, during which `callTool`/`listTools` fail fast with `SERVICE_UNAVAILABLE` (`circuit_opened` / `circuit_fail_fast` debug events). Success resets the counter; caller-driven aborts never count.
- Abort signals propagate end to end: the browser lane threads the tRPC request signal (`ctx.signal`, lambda context) and the durable worker lane threads the composed run/`abortController` signal through `executeConversationToolStep` → `invokeConversationTool` → `MCPService.callTool` → SDK request options. An aborted worker call is rethrown rather than persisted as a tool error, so stop/cancel classification stays fence-based.
...
- Model-boundary hygiene (`src/server/services/mcp/metadata.ts`): tool descriptions are server-controlled untrusted text, so `listTools` scrubs classic prompt-injection imperatives (`ignore/disregard … previous|prior|above instructions` → `[redacted MCP metadata instruction]`, `system prompt` phrase normalized) and caps descriptions at 1200 chars before manifests are synthesized. `callTool` normalization strips protocol-reserved `_meta` (top level + content items; tool-owned `structuredContent`/text payloads are untouched). `ToolsEngine.convertManifestsToTools` sorts the final tool list by resolved name with a code-unit comparator so the tools block is byte-stable across turns regardless of plugin-id ordering (prompt-cache friendly; `localeCompare` would vary with ICU locale).
