...
Xiaomi MiMo uses `MIMO_API_KEY` (enables `ENABLED_MIMO`) and optional `MIMO_PROXY_URL`. The built-in Chat Completions host is `https://api.xiaomimimo.com/v1`. Token Plan keys (`tp-...`) need `MIMO_PROXY_URL=https://token-plan-cn.xiaomimimo.com/v1`. Request/user-provider `baseURL` overrides the env proxy URL. Put these in the **container** env (Compose / systemd), not only the host `~/.bashrc`. Token Plan omits native `{ type: web_search }` and keeps ChatHub/MCP search even if the native-search toggle was on. The search panel hides that toggle when Settings has a Token Plan base URL **or** when only `MIMO_PROXY_URL` is Token Plan (`serverConfig.mimoTokenPlanEnv`). Structured-output jobs use Xiaomi `json_object` mode (not OpenAI `json_schema`); the tools/`generateObject` path uses the same JSON mode so group supervisors cannot silently no-op on a text reply. Cache-hit telemetry (`usage.prompt_tokens_details.cached_tokens`) needs `DEBUG_MIMO_CACHE=1` on the same container to emit `model-cache-debug`.
