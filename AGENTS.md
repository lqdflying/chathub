...
- **Never** `git push`, create/push a tag, trigger a remote build, or perform GA/release actions without an **explicit in-turn** user instruction. **Exception: the separate `wiki/` repository**, which has standing permission — commit and push it directly whenever `git -C wiki push --dry-run origin master` succeeds, and only fall back to the patch handoff in `CLAUDE.md` when that dry run is actually denied. Never infer "this must be a cloud session" without running it.
