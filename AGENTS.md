...
- **Never** `git push`, create/push a tag, trigger a remote build, or perform GA/release actions without an **explicit in-turn** user instruction. **No exceptions, including the separate `wiki/` repository** — write access to that clone is not permission to publish — **with one standing exception: an explicit canary-flow instruction includes publishing the wiki's committed commits alongside the tag** (`.cursor/rules/git-and-release.mdc` *Fix / feature canary flow*). Otherwise, edit and commit `wiki/` in place (never hand over a `format-patch` file as the routine deliverable), then stop and report; the patch handoff in `CLAUDE.md` is only for a push you were told to make and that was denied.
...
Overview, stack, naming, wiki workflow: **`.cursor/rules/project.mdc`**. **B/S split (server-first, thin browser):** **`.cursor/rules/architecture.mdc`**. Per-model API toggles (extendParams): **`.cursor/rules/provider-model-options.mdc`**. Native MiniMax live probes: **`MINIMAX_API_KEY`** / **`MINIMAX_PROXY_URL`** in `~/.bashrc` (**`.cursor/rules/minimax-live-probe.mdc`**). Provider brand icons (always vendor offline): **`.cursor/rules/provider-icons.mdc`**. Commands and test constraints: **`.cursor/rules/commands.mdc`**. MCP management UI patterns: **`.cursor/rules/mcp-management-ui.mdc`**. Claude-like leave-and-keep-going generation: **`.cursor/rules/durable-background-generation.mdc`**. Code Interpreter product prompt vs assistant/sidecar packages: **`.cursor/rules/code-interpreter-prompt.mdc`**. Code Interpreter / DifySandbox jail changes: reproduce on a local sidecar image first (**`.cursor/rules/code-interpreter-sandbox-repro.mdc`**).
