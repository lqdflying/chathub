/**
 * Idempotent repair for chat-group memberships and their ownership constraints.
 * Safe to run after every migration, including when an earlier repair partially ran.
 */
...
const ensureChatGroupMembershipOwnership = async (client) => {
  await client.query(CHAT_GROUP_MEMBERSHIP_OWNERSHIP_SQL);
};

module.exports = {
  CHAT_GROUP_MEMBERSHIP_OWNERSHIP_SQL,
  ensureChatGroupMembershipOwnership,
};
