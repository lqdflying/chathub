...
- Cache troubleshooting debug:
  - `DEBUG_OPENAICOMPATIBLE_CACHE=1` logs redacted Chat/Responses endpoint and request fingerprints, turn/input shapes, tool count/fingerprint, cache-key/session-header hashes, hashed response IDs, and final cached-token usage when usage is available.
